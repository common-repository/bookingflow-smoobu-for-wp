...
final class Smoobu_Settings {
	...
	public function register_menu() {
		// main page.
		add_menu_page( __( 'General Settings', 'smoobu-calendar' ), __( 'Smoobu Calendar', 'smoobu-calendar' ), 'manage_options', 'smoobu-calendar-settings', array( $this, 'show_general_settings' ), 'dashicons-clipboard', 107 );
		...
		if ( isset( $_POST['smoobu_settings_nonce'] ) && wp_verify_nonce( sanitize_key( $_POST['smoobu_settings_nonce'] ), 'smoobu_settings_nonce' ) ) {
			...
			if ( isset( $_POST['smoobu_general_settings_save'] ) ) {
				...
				if ( false === $check_result ) {
					...
					if ( ! empty( $properties_error ) || ! empty( $availability_error ) ) {
						return array(
							'success' => false,
